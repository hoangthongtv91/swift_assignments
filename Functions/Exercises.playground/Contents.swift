...
func lastLetter(Of string: String) -> Character{
    var str = string
    return str.remove(at: str.index(before: str.endIndex))
}
//:Test out your discovery below by returning the last letter of the String, "bologna".
var word = "man"
print(lastLetter(Of: word))
...
func stringContainsOnlyDigits(_ string:String) -> Bool{
    var FLAG = true;
...
let str = "109ad"
print(stringContainsOnlyDigits(str))
...
func removeDirtyWords(from array:[String]) -> [String]{
    var result:[String] = []
    for i in array{
        if i.count != 4{
            result.append(i)
...
class MovieArchive {
    func filterByDirector(_ name:String, from dictionary:[String:String]) -> [String] {
        var result:[String] = []
        for (film, director) in dictionary{
            if director == name{
                result.append(film)
            }
        }
